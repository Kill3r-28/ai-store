...
2. Add redirect URIs: `http://localhost:8501/` and `https://<your-app>.streamlit.app/` (app root, **not** `/oauth2callback`).
3. On **Streamlit Cloud**, paste secrets from `docs/streamlit-cloud-secrets.toml` — **no `[auth]` section**.
4. Set `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `OAUTH_STATE_SECRET`, and `APP_PUBLIC_URL`.
5. Run `python scripts/verify_oauth.py` locally before deploy.
